...
function check(win, change) {
    ...
    if (
        ...
        if (_old_workspaces[name] !== undefined) {
            ...
            if (w.length == 0) {
                // TODO expose this as user choice
...
function handleWindowClose(act) {
    ...
    if (_old_workspaces[name] !== undefined) {
        win.get_display()
            ...
            .get_workspace_by_index(_old_workspaces[name])
    ...
    _window_manager_handles.push(
        global.window_manager.connect("size-change", (_, act, change) => {
            ...
            GLib.timeout_add(
                GLib.PRIORITY_LOW,
                300,
                check.bind(this, act.meta_window, change)
            );
        })
    );
    _window_manager_handles.push(
